...
<body>
  <div class="main-card">
    ...
    <form id="download-form" autocomplete="off" method="post">
      ...
      <div id="audio-quality" style="display:none;">
    ...
    <img id="thumb-preview" src="#" alt="Thumbnail" style="display:none;"/>
    <div id="download-links">
      <a id="video-link" href="#" download target="_blank">⬇️ Save File</a>
    </div>
  ...
  <script>
    ...
    let downloadId=null, timerId=null;
    ...
    function pollProgress() {
      fetch("/progress/" + downloadId).then(res=>res.json()).then(data=>{
        document.getElementById("progress-bar").style.width = data.percent + "%";
        document.getElementById("progress-bar").innerText = data.percent + "%";
        if (data.finished) {
          clearInterval(timerId);
          document.getElementById('download-btn').disabled = false;
          document.getElementById('qr-code').src = "/qrcode/" + encodeURIComponent(data.filename);
          document.getElementById('qr-code').style.display = "block";
          document.getElementById('video-link').href = "/download/" + encodeURIComponent(data.filename);
...
def index():
    if request.method == "POST":
        ...
        q = request.form.get("mp4quality", "720")
        download_id = str(uuid.uuid4())
        ydl_opts = {
            'format': f'bestvideo[height<={q}]+bestaudio/best' if fmt == 'mp4' else 'bestaudio/best',
...
def qrcode_route(filename):
    ...
    img = qrcode.make(f"{public_url}download/{filename}")
    buf = io.BytesIO()
    img.save(buf, "PNG")
    buf.seek(0)
