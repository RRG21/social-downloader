...
import yt_dlp
import requests
# Added 'send_from_directory' to imports for PWA compatibility
from flask import Flask, request, render_template_string, jsonify, send_file, redirect, abort, send_from_directory
...
if ("serviceWorker" in navigator) {
  // Changed path to root level for service worker scope
...
def progress_hook(d):
    ...
    if status == "downloading":
        ...
        percent = "{:.1f}".format(downloaded / total * 100) if total else "0"
        prog.update({"percent": percent, "downloaded_bytes": downloaded, "total_bytes": total})
...
def index():
    if request.method == "POST":
        ...
        mp3quality = request.form.get("mp3quality", "m4a_128")
        mp4quality = request.form.get("mp4quality", "720")
        title, ext, thumb_url = fetch_video_title_and_thumb(url, fmt, mp3quality, mp4quality)
        safe_title = title
        download_id = str(uuid.uuid4())
        out_path = os.path.join(DOWNLOAD_FOLDER, safe_title)
        download_info[download_id] = {"title": safe_title, "extension": ext}
        def run_down():
            try:
                download_video(url, fmt, mp3quality, mp4quality, out_path, download_id)
                for ext_i in ["mp3", "m4a", "mp4"]:
                    candidate = out_path + "." + ext_i
                    if os.path.exists(candidate):
                        download_progress[download_id]["filename"] = os.path.basename(candidate)
                        break
            except Exception as e:
                download_progress[download_id] = {"error": str(e)}
        download_progress[download_id] = {"percent": "0", "downloaded_bytes": 0, "total_bytes": 0}
        threading.Thread(target=run_down).start()
        return jsonify({"download_id": download_id, "title": safe_title, "extension": ext, "thumb_url": thumb_url})
...
def progress(download_id):
    ...
    if not prog:
        return jsonify({"percent": "0", "downloaded_bytes": 0, "total_bytes": 0})
    if "filename" in prog:
        prog["finished"] = True
    return jsonify(prog)
...
def download(filename):
    ...
    info = None
    for d_id, dp in download_progress.items():
        if dp.get("filename") == filename:
            info = download_info.get(d_id)
            break
    download_name = filename
    if info and info.get("title") and info.get("extension"):
        download_name = f"{info['title']}.{info['extension']}"
...
def getthumb():
    ...
    if not url:
        return "No thumbnail url", 404
    try:
        resp = requests.get(url, timeout=15)
        if resp.status_code == 200:
            return send_file(io.BytesIO(resp.content), mimetype="image/jpeg", as_attachment=True, download_name="thumbnail.jpg")
    except Exception:
        pass
    return "Failed to download thumbnail!", 500
...
def qrcode_route(filename):
    ...
    try:
        ...
        s.close()
    file_url = f"http://{ip}:5000/download/{filename}"
...
# --- NEW: Production routes for PWA/APK conversion ---
